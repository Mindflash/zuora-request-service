...
Zuora Request Service is a promise based library to facilitate interaction with the [zuora](https://www.zuora.com/developer/api-reference/#tag/HMAC%20Signatures) http API.
This service provides several conveniences for interacting with the zuora api:
+ The service will automatically append auth and the appropriate content type headers to every out going request,
+ Handles constructing all the URL's for the zuora API
+ Intercepts pesky errors and failures that are returned from zuora as 200's and automatically rethrows those as proper errors for you
+ Allows you to configure requests to retry under specific conditions
+ Handles all the boiler plate for logging your interactions out with zuora
+ Provides a configurable time out feature on your requests to keep your processes from hanging if zuora becomes unresponsive. 
...
zuora.setup('dummyUser', 'dummyPassword', yourLogger, false);
...
request options object properties:
  ...
  + body: object - request body required for most POST and PUT http calls.
